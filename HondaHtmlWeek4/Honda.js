...
// import * as chroma from 'https://cdn.jsdelivr.net/npm/chroma-js@2/chroma.min.js';
...
function generateByMaterial(filterData,action){
  ...
  filterData.forEach(d => {
    ...
    let textNode = {name: `${d["Short Text"]}`};
...
function drawSankeyDiagram(nodes, links, action){
    ...
    const color = d3.scaleOrdinal()
      .domain(graphNodes.map(d => d.name))  // domain is the list of node names
      .range(graphNodes.map((_, i) => d3.interpolateRainbow(i / graphNodes.length)));
...
// Following part is a test function if sankey plot with zoom in and out function,
// not good enough
...
//   const color = d3.scaleOrdinal()
//     .domain(graphNodes.map(d => d.name))
//     .range(graphNodes.map((_, i) => d3.interpolateRainbow(i / graphNodes.length)));

//   const g = svg.append("g");  // Container for the diagram to apply zoom transformations

//   const zoom = d3.zoom()
//       .scaleExtent([0.5, 2])  // Limit zoom scale for better control
//       .on("zoom", event => {
//           g.attr("transform", event.transform);  // Update the transformation
//       });
